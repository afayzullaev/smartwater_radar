...
/**
 * A9G related custom AT commands
*/

extern "C" esp_err_t esp_modem_get_ip_addr(esp_modem_dce_t *dce_wrap, char *p_ip)
{
    if (dce_wrap == nullptr || dce_wrap->dce == nullptr) {
        return ESP_ERR_INVALID_ARG;
    }
    std::string ip;
    auto ret = command_response_to_esp_err(dce_wrap->dce->get_ip_addr(ip));
