...
command_result at(CommandableIf *t, const std::string &cmd, std::string &out, int timeout = 2000)
{
    ESP_LOGV(TAG, "%s", __func__ );
...
command_result set_gnss_power_mode_sim76xx(CommandableIf *t, int mode)
{
    ...
    return generic_command(t, "AT+CGNSSPWR=" + std::to_string(mode) + "\r", "READY", "ERROR", 3000);
    // +CGNSSPWR: READY!
}


command_result get_ip_addr(CommandableIf *t, std::string &out)
{
    ESP_LOGV(TAG, "%s", __func__ );
    return generic_get_string(t, "AT+CIFSR\r", out, 5000);
}
